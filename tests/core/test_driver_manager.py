...
@pytest.mark.parametrize(
    ...
    ids=(
        "bypass brave -> chrome driver",
        "bypass chrome -> chrome driver",
        "bypass chromium -> chrome driver",
        "bypass edge -> edge driver",
    ),
)
@mock.patch("udemy_enroller.driver_manager.webdriver")
@mock.patch("udemy_enroller.driver_manager.EdgeOptions")
...
@mock.patch("udemy_enroller.driver_manager.ChromeService")
@mock.patch.object(
    DriverManager, "_get_chromium_version", return_value="120.0.6099.109"
)
@mock.patch.object(DriverManager, "_wait_for_port", return_value=True)
@mock.patch.object(
    DriverManager, "_resolve_browser_binary", return_value="/fake/browser"
)
def test_driver_manager_cloudflare_bypass(
    mock_resolve_binary,
    mock_wait_for_port,
    mock_get_version,
    mock_chrome_service,
    ...
    dm = DriverManager(browser_name, cloudflare_bypass=True)

    # Binary is resolved once, and _wait_for_port returning True means we attach
    # to a "running" instance (no subprocess launch needed).
    mock_resolve_binary.assert_called_once_with(browser_name)
    assert mock_wait_for_port.called

    if expected_driver == "Edge":
        assert mock_selenium_web_driver.Edge.called
        assert dm.driver == mock_selenium_web_driver.Edge()
        mock_selenium_web_driver.Chrome.assert_not_called()
    else:
        assert mock_selenium_web_driver.Chrome.called
        assert dm.driver == mock_selenium_web_driver.Chrome()
        mock_selenium_web_driver.Edge.assert_not_called()
