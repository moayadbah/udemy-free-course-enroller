    ...
    def login(self, is_retry=False) -> None:
        ...
        if not self.logged_in:
            ...
            try:
                ...
                # Udemy now uses a multi-step login flow, frequently shows a
                # Cloudflare check, and may enforce 2FA. Rather than trying to
                # automate every variation, fall back to manual login in the
                # browser window. This handles email/password, captcha and the
                # 6-digit email code all in one step.
                logger.info(
                    "Could not auto-fill the login form (Udemy changed its login page)."
                )
                self._manual_login(user_dropdown_xpath)
                return
            else:
                try:
                    ...
                    # Captcha cleared but still not logged in -> likely 2FA or
                    # some other verification step. Let the user finish manually.
                    self._manual_login(user_dropdown_xpath)
                    return
                self.logged_in = True

    def _manual_login(self, user_dropdown_xpath: str) -> None:
        """
        Pause and let the user complete login manually in the browser window.

        Handles Udemy's multi-step login, Cloudflare checks and 2FA (the
        6-digit email code) without needing to automate each variation.

        :param str user_dropdown_xpath: XPath that only appears once logged in
        :return: None
        """
        input(
            "\n>>> Please log in to Udemy in the browser window that just opened.\n"
            ">>> Complete every step: email, password, any captcha, and the\n"
            ">>> 6-digit code Udemy emails you for two-factor verification.\n"
            ">>> Once you can see your Udemy homepage (logged in), come back\n"
        ...
        try:
            ...
            raise LoginException(
                "Still not logged in. Make sure you completed login (including the "
                "2FA code) in the browser window before pressing Enter."
            )
