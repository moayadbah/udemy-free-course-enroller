...
class UdemyActions:
    ...
    def login(self, retry=False) -> None:
        ...
        Handles 2FA email OTP if Udemy requires it.

        :return: None
        """
        cookie_details = self._load_cookies()

        # Track whether we are using cookies that were loaded from disk (e.g.
        # bridged from a browser session via bridge_cookies.py). If so we don't
        # want to silently delete them and fall back to the REST login flow,
        # which Udemy now blocks with Cloudflare + 2FA.
        using_saved_cookies = (
            cookie_details is not None and "access_token" in cookie_details
        )
        ...
        if cookie_details is None:
            ...
            if auth_response.status_code == 200:
                ...
                # Udemy sends a 6-digit OTP to email when 2FA is required.
                # In that case access_token is absent from the initial login response
                # and the response JSON contains a returnUrl pointing to /join/two-factor/.
                if "access_token" not in cookie_details:
                    return_path = the_json.get("returnUrl", "")
                    if not return_path:
                        raise Exception(
                            "Udemy requires OTP but returned no 'returnUrl'. "
                            f"Full login response: {the_json}"
                        )
                    two_factor_url = f"{self.BASE_URL}{return_path}"

                    # GET the two-factor page so cloudscraper can satisfy Cloudflare
                    # and we receive a fresh csrftoken bound to this session.
                    tf_page = self.udemy_scraper.get(
                        two_factor_url,
                        cookies=cookie_details,
                        headers={**self._headers, "Referer": self.LOGIN_URL},
                    )
                    session_cookies = {**cookie_details, **dict(tf_page.cookies)}
                    otp_csrf = session_cookies.get("csrftoken", csrf_token)
                    ...
                    otp_response = self.udemy_scraper.post(
                        ...
                        data={
                            "otp": otp_code,
                            "csrfmiddlewaretoken": otp_csrf,
                        },
                        cookies=session_cookies,
                        headers={**self._headers, "Referer": two_factor_url},
                        allow_redirects=False,
                    )

                    if otp_response.status_code in (200, 302):
                        merged = {**session_cookies, **dict(otp_response.cookies)}
                        cookie_details = merged
                    else:
                        raise Exception(
                            f"OTP verification failed (HTTP {otp_response.status_code}): "
                            f"{otp_response.text[:300]}"
                        )

                if "access_token" not in cookie_details:
                    raise Exception(
                        "Login succeeded but no access_token was returned. "
                        "The OTP may be incorrect or the Udemy session has expired."
                    )
        ...
        try:
            ...
            if isinstance(e, requests.HTTPError):
                ...
                logger.error(e)
                retry = True
            # Never auto-delete cookies that were bridged from a browser session.
            # The REST login fallback can't get past Udemy's Cloudflare + 2FA, so
            # deleting them would just lose a working session for nothing.
            if using_saved_cookies:
                logger.error(
                    "The saved Udemy cookies did not work (they may have expired). "
                    "Re-run `python3 bridge_cookies.py` to refresh them from the browser."
                )
                raise e
            if not retry:
    ...
    def _get_course_id(self, url: str) -> int:
        ...
        # Udemy embeds the course id in a mobile deeplink, e.g.
        # "urlMobileNativeDeeplink":"udemy://discover?courseId=6182265"
        match = re.search(r"courseId=(\d+)", html)
        if match:
            return int(match.group(1))

        # Fallback to the legacy body attribute in case the page format reverts
        soup = BeautifulSoup(response.content, "html.parser")
        body = soup.find("body")
        if body is not None and body.has_attr("data-clp-course-id"):
            return int(body["data-clp-course-id"])

        raise Exception(f"Unable to determine course id from page: {url}")
